...
public class WorkableSpotsGenerator : MonoBehaviour {
	...
	void Start() {
		...
		Vector3 position = center - new Vector3((tiles - 1) / 2, 0, tiles / 2 + offset);
		...
		position = center - new Vector3((tiles - 1) / 2, 0, - (tiles / 2 + offset));
		for (int i = 0; i < tiles - 1; i++) { 
			GameObject indicator = Instantiate (indicatorPrefab, position, Quaternion.identity);
			indicator.transform.SetParent (indicatorParent);
		...
		position = center - new Vector3( - (tiles / 2 + offset), 0, (tiles - 1) / 2);
		for (int i = 0; i < tiles - 1; i++) { 
			GameObject indicator = Instantiate (indicatorPrefab, position, Quaternion.identity);
			indicator.transform.SetParent (indicatorParent);
